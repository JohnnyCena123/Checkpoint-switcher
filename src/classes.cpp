...
bool CheckpointSelectorButton::init(int buttonID, MyCheckpointObject* checkpoint) {
    ...
    m_buttonLabel = CCLabelBMFont::create(m_checkpoint ? fmt::format("Checkpoint at {:.4}%", m_checkpoint->m_fields->m_currentPrecentage).c_str() : "The start!", "bigFont.fnt"); /* nullptr check */ if (!m_buttonLabel) {log::error("button label failed to initialize."); hasFailed = true;}
