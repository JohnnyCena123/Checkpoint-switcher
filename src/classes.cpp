...
class $modify(MyPlayLayer, PlayLayer) {
    ...
    static void onModify(auto& self) {
        log::info("Hooking PlayLayer...");
        if (!self.setHookPriorityPre("PlayLayer::resume", Priority::Last)) {
            log::warn("Failed to set hook priority for PlayLayer::resume.");
        }
        if (!self.setHookPriorityPre("PlayLayer::resumeAndRestart", Priority::Last)) {
            log::warn("Failed to set hook priority for PlayLayer::resumeAndRestart.");
        }
        if (!self.setHookPriorityPre("PlayLayer::resetLevel", Priority::Last)) {
            log::warn("Failed to set hook priority for PlayLayer::resetLevel.");
        }
    }
    ...
    // void resume() {
    //     if (m_fields->m_selectedCheckpoint != nullptr && isSwitcherOn) {
    //         PlayLayer::loadFromCheckpoint(m_fields->m_selectedCheckpoint);
    //     } else {
    //         resume();
    //     }
    // }
...
bool CheckpointSwitcherLayer::setup() {
    ...
    m_mainLayer->addChildAtPosition(m_toggleSwitcherButtonLabel, Anchor::BottomRight, ccp(20.f + m_toggleSwitcherButton->getContentWidth(), 10.f));
...
bool CheckpointSelectorButton::init(int buttonID, CheckpointObject* checkpoint) {
    ...
    if (!m_checkpointGlowOutline) {log::error("checkpoint glow outline failed to initialize."); return false;}
    ...
    if (!m_buttonLabel) {log::error("button label failed to initialize."); return false;}
    m_checkpointSprite->addChild(m_buttonLabel);
    m_buttonLabel->setPosition(m_checkpointSprite->getContentWidth() / 2.f, m_checkpointSprite->getContentHeight() + 5.f);
...
void CheckpointSelectorButton::onSelectButton(CCObject* sender) {
    ...
    if (lastSelectedButton != nullptr) lastSelectedButton->runAction(CCEaseInOut::create(CCScaleBy::create(0.1f, 0.8f), 2.f));
